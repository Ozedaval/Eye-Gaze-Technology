...
import com.pwc.explore.databinding.ActivityEventTestTrackerBinding;
...
//
 static{ System.loadLibrary( "opencv_java4" );}
    ...
    private static final String TAG="EyeGazeEventActivity2";
    private RealTimeTest realTimeTest;
    private ActivityEventTestTrackerBinding binding;

    @Override
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        binding=ActivityEventTestTrackerBinding.inflate(getLayoutInflater());
    ...
    public Mat onCameraFrame(CameraBridgeViewBase.CvCameraViewFrame inputFrame) {
      binding.eventTextView.post(new Runnable() {
            ...
            public void run() {
                ...
                if (detect.lookdown==true){binding.lookDown.setText("BOTTOM");}
                else {binding.lookDown.setText("NEUTRAL");}
            }
        });
    ...
    public void BottomButton(View view) {
        realTimeTest.setNumOfTry(realTimeTest.getNumOfTry() + 1);
        if (binding.lookDown.toString().equals("BOTTOM"))
            realTimeTest.setNumOfCorrect(realTimeTest.getNumOfCorrect() + 1);
        realTimeTest.setAccuracy();
        binding.calculatedAccTextView.setText(realTimeTest.getAccuracy() +" %");
        if (realTimeTest.getNumOfTry() % 100 == 0) {
